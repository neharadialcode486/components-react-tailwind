                    ...
                    <div className="max-w-[779px] relative z-10 w-full mx-auto flex md:justify-center items-center gap-4">
                        {TABS_DATA.map((tab, idx) => (
                            <a href={`/${tab.title.toLowerCase()}`} key={idx}
                                ...
                                <span className={activeTab === idx ? "text-transparent bg-linear-green bg-clip-text font-inter" : "text-white font-inter"}>{tab.title}</span>
                            </a>
                        ))}
                    </div>
                </div>
                            ...
                            <div key={index} className={`overflow-hidden flex mt-2.5 sm:mt-6 justify-center relative duration-300 ease-linear border rounded-2xl ${openIndex === index ? "border-off-green" : "border-light-gray"}`} >
                                <div className="accordion w-full">
                                    <div role="button"
                                        ...
                                        <h3 className="font-medium text-off-white font-inter text-base sm:text-lg" >{item.title}</h3>
                                        <div className={`${openIndex === index ? 'rotate-180' : ''} w-3 sm:w-5 duration-300 ease-linear`}><Icon iconName='plusIcon' /></div>
                                    </div>
                                    <div ref={(el) => { contentRefs.current[index] = el; }}
                                        ...
                                        <div className="flex flex-col p-6">
                                            <p className="text-dark-gray text-sm sm:text-base font-medium font-inter">{item.content}</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
